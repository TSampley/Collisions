...
//    				moveForward(q); p.moveForward(q);
    ...
    {
    	return Math.sqrt(Math.pow((one.origin.x - q*one.momentum.x/one.mass) - (two.origin.x - q*two.momentum.x/two.mass), 2) + Math.pow((one.origin.y - q*one.momentum.y/one.mass) - (two.origin.y - q*two.momentum.y/two.mass), 2) + Math.pow((one.origin.z - q*one.momentum.z/one.mass) - (two.origin.z - q*two.momentum.z/two.mass), 2)) - one.radius - two.radius;
    }
